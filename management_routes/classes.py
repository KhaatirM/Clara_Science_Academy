...
def add_class():
    ...
    if request.method == 'POST':
        try:
            ...
            # School-managed Google Group + Classroom (botadmin creates course, direct-enrolls roster).
            try_provision_class_google_group(new_class.id)
            db.session.refresh(new_class)
            if new_class.google_classroom_id:
                flash(
                    f'Class "{name}" created successfully. Google Classroom was provisioned and rostered.',
                    'success',
                )
            else:
                flash(
                    f'Class "{name}" created successfully. Google Classroom will appear once school Google admin is configured / synced.',
                    'info',
                )
