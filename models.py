...
class ReportCard(db.Model):
    ...
    # True when generated by the automated year-end closure job, false/null for manual.
...
class Grade3StandardMark(db.Model):
    """
    Teacher-entered marks for the 3rd grade Language Arts / Math standards checklist.
    Stored per-student, per-standard, per-school-year, per-quarter.
    `standard_id` is a string key (e.g. 'la_cr_1', 'math_no_2') matching the
    catalog in utils/report_card_grade3_standards.py.
    `mark` is one of: 'M', 'W', 'NA', 'UA'.
    """
    __tablename__ = 'grade3_standard_mark'
    ...
    student = db.relationship('Student', backref='grade3_standard_marks', lazy=True)
    school_year = db.relationship('SchoolYear', backref='grade3_standard_marks', lazy=True)
    editor = db.relationship('User', backref='edited_grade3_standard_marks', lazy=True)

    __table_args__ = (
        db.UniqueConstraint(
            'student_id', 'standard_id', 'school_year_id', 'quarter',
            name='uq_grade3_standard_mark_key',
        ),
    )
...
# ============================================================
# Phased school-year closure workflow
# (Backs the Day 0 → Day 7 → Day 21 → Day 28 workflow scheduled
# from the management Report Cards page.)
# ============================================================

class SchoolYearClosure(db.Model):
    """
    One row per scheduled "close school year" workflow.

    Phases (state machine):
      scheduled     — created, before student notice goes out
      student_window — Day 0..Day 7: students notified, can still submit work
      teacher_window — Day 7..Day 21: students locked (hybrid hidden), teachers active
      admin_window   — Day 21..Day 28: teachers locked, admins finalize
      finalized      — Day 28+: report cards generated, year archived, students promoted
      paused         — workflow paused by director (transitions frozen, dates don't slide)
      cancelled      — workflow aborted before finalization
    """
    __tablename__ = 'school_year_closure'

    id = db.Column(db.Integer, primary_key=True)
    school_year_id = db.Column(db.Integer, db.ForeignKey('school_year.id'), nullable=False)

    closure_date = db.Column(db.Date, nullable=False)  # "Day 0" — set by director
    student_lockout_at = db.Column(db.Date, nullable=False)  # default closure_date + 7
    teacher_lockout_at = db.Column(db.Date, nullable=False)  # default closure_date + 21
    finalize_at = db.Column(db.Date, nullable=False)         # default closure_date + 28

    phase = db.Column(db.String(20), nullable=False, default='scheduled')
    previous_phase = db.Column(db.String(20), nullable=True)  # for pause/resume

    created_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    student_notice_sent_at = db.Column(db.DateTime, nullable=True)
    teacher_notice_sent_at = db.Column(db.DateTime, nullable=True)
    admin_warning_sent_at = db.Column(db.DateTime, nullable=True)

    students_locked_at = db.Column(db.DateTime, nullable=True)
    teachers_locked_at = db.Column(db.DateTime, nullable=True)
    ...
    paused_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)

    notes = db.Column(db.Text, nullable=True)
    last_tick_at = db.Column(db.DateTime, nullable=True)

    # Outcome stats (JSON; populated on finalize for the dashboard)
    finalize_stats = db.Column(db.Text, nullable=True)

    school_year = db.relationship('SchoolYear', backref='closures')
    created_by = db.relationship('User', foreign_keys=[created_by_user_id])
    cancelled_by = db.relationship('User', foreign_keys=[cancelled_by_user_id])
    paused_by = db.relationship('User', foreign_keys=[paused_by_user_id])

    @property
    def is_active(self):
        return self.phase not in ('finalized', 'cancelled')
    ...
    def __repr__(self):
        return f"SchoolYearClosure(year={self.school_year_id}, phase={self.phase}, closure={self.closure_date})"


class SchoolYearClosureExtension(db.Model):
    """
    Per-user or per-class deadline extension during a year-end closure.

    Scope (set at least one):
      - scope_user_id: extension applies only to this user (student or teacher).
      - scope_class_id: extension applies to everyone associated with this class.

    for_role controls which lockout it extends:
      - 'student' — pushes student_lockout_at for this scope
      - 'teacher' — pushes teacher_lockout_at for this scope
      - 'both'   — pushes both (typical for per-class extensions)
    """
    __tablename__ = 'school_year_closure_extension'

    id = db.Column(db.Integer, primary_key=True)
    closure_id = db.Column(db.Integer, db.ForeignKey('school_year_closure.id'), nullable=False)

    scope_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)
    scope_class_id = db.Column(db.Integer, db.ForeignKey('class.id'), nullable=True)
    for_role = db.Column(db.String(10), nullable=False, default='both')  # student|teacher|both

    extended_until = db.Column(db.Date, nullable=False)
    reason = db.Column(db.Text, nullable=True)
    ...
    closure = db.relationship('SchoolYearClosure', backref='extensions')
    scope_user = db.relationship('User', foreign_keys=[scope_user_id])
    scope_class = db.relationship('Class', foreign_keys=[scope_class_id])
    ...
    def __repr__(self):
        return (
            f"SchoolYearClosureExtension(closure={self.closure_id}, "
            f"user={self.scope_user_id}, class={self.scope_class_id}, "
            f"role={self.for_role}, until={self.extended_until})"
        )


class SchoolYearClosureEvent(db.Model):
    """
    Audit log: one row per transition / action / notification fired against a closure.

    event_type examples:
      created, scheduled, phase_advanced, paused, resumed, postponed,
      cancelled, finalized, reopened,
      extension_granted, extension_revoked,
      notification_sent, notification_failed
    """
    __tablename__ = 'school_year_closure_event'
    ...
    actor_label = db.Column(db.String(40), nullable=True)  # 'system'|'cron'|'manual' when actor is null
    payload = db.Column(db.Text, nullable=True)  # JSON
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    closure = db.relationship('SchoolYearClosure', backref='events')
