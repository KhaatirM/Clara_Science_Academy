...
def update_all_gpas():
    ...
    with app.app_context():
        ...
        print(f"[{datetime.now()}] Starting active-year GPA update for all students...")
        try:
            result = sync_active_year_gpas(commit=True, force=True)
            print(
                f"[{datetime.now()}] GPA update completed. "
                f"year_id={result.get('school_year_id')} "
                f"updated={result.get('updated')} cleared={result.get('cleared')}"
            )
        except Exception as e:
            print(f"[{datetime.now()}] GPA update error: {e}")
            raise
