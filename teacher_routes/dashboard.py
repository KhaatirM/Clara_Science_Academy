...
def teacher_dashboard():
    ...
    if student_ids:
        ...
        # Get group grades for students in teacher's classes
        group_grades = []
        if group_assignments:
            group_assignment_ids = [ga.id for ga in group_assignments]
            # Get all groups for these assignments
            groups = []
            if class_ids:
        ...
        missing_assignments = []
        if class_ids:
            # Get all active assignments for teacher's classes
            ...
            # For each assignment, check if each student has a grade
    ...
    for grade in at_risk_grades:
        try:
            # Check if assignment and student exist
            if not grade.assignment or not grade.student:
                continue
            ...
            if is_at_risk:
                if grade.student.id not in seen_student_ids:
                    # Safely get assignment_type with fallback
                    assignment_type = grade.assignment.assignment_type if grade.assignment.assignment_type else 'pdf'
    ...
    # Calculate summary statistics for academic alerts
    failing_count = sum(1 for alert in at_risk_alerts if 'failing' in alert['alert_reason'].lower())
    overdue_count = sum(1 for alert in at_risk_alerts if 'overdue' in alert['alert_reason'].lower())
