"""
1st grade standards checklist editor.
...
bp = Blueprint('grade1_standards', __name__)
...
def _is_first_grade_class(class_obj: Class) -> bool:
    if not class_obj:
        return False
    try:
        ...
        levels = []
    return 1 in [int(l) for l in levels if str(l).isdigit() or isinstance(l, int)]
...
def _classes_for_user(school_year_id: int):
    ...
    classes = []
    for c in candidates:
        if not _is_first_grade_class(c):
            continue
...
def _decorate_class(class_obj, subject_key, school_year_id):
    ...
    class_obj.g1_stats = stats
    class_obj.g1_subject_key = subject_key
    class_obj.g1_student_count = len(students)
    return class_obj


@bp.route('/grade1-standards', methods=['GET'])
@login_required
@teacher_required
def grade1_standards_index():
    ...
    total_students = sum(c.g1_student_count for c in classes)
    overall_filled = sum(c.g1_stats['overall']['filled'] for c in classes)
    overall_total = sum(c.g1_stats['overall']['total'] for c in classes)
    ...
    return render_template(
        'teachers/grade1_standards_index.html',
...
@bp.route('/grade1-standards/<int:class_id>', methods=['GET', 'POST'])
@login_required
@teacher_required
def grade1_standards_editor(class_id: int):
    from utils.spa_management_urls import user_should_use_spa_management_shell
    ...
    if (
        ...
        path = f'/app/management/report-cards/standards/grade1/{class_id}'
        params = []
        if quarter:
    ...
    if not is_authorized_for_class(class_obj):
        abort(403)
    if not _is_first_grade_class(class_obj):
        flash('This is not a 1st grade class.', 'warning')
        return redirect(url_for('teacher.grade1_standards.grade1_standards_index'))

    subject_key = _class_subject_key(class_obj)
    if not subject_key:
        flash(
            'This class subject is not Language Arts or Math, '
            'so it cannot use the 1st grade standards checklist.',
            'warning',
        )
        return redirect(url_for('teacher.grade1_standards.grade1_standards_index'))
    ...
    if request.method == 'POST':
        ...
        if bulk_action == 'copy_previous':
            ...
            return redirect(url_for('teacher.grade1_standards.grade1_standards_editor', class_id=class_id, quarter=quarter))
        ...
        return redirect(url_for(
            'teacher.grade1_standards.grade1_standards_editor',
            class_id=class_id,
            quarter=quarter,
    ...
    return render_template(
        'teachers/grade1_standards_editor.html',
        class_obj=class_obj,
        subject_key=subject_key,
