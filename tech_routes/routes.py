...
@tech_required
def resources():
    """MinuteMath removed from SPA-only main (kept on archive/* branches)."""
    flash(
        'Curriculum resource files are no longer hosted in-app. '
        'See archive/hybrid-spa-migration or archive/legacy-pre-spa for MinuteMath.',
        'info',
    )
...
def download_resource(filename):
    """MinuteMath downloads removed from SPA-only main."""
    flash('Resource downloads are no longer available in this deployment.', 'warning')
    return redirect('/app/tech')
